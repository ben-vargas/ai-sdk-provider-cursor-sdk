/**
 * Maintainer-only manual smoke for the provider's live AI SDK v6 result contract.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const firstMetadata = first.providerMetadata?.cursor;
    ...
    const streamedMetadata = (await streamed.providerMetadata)?.cursor;
